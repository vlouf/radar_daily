...
def daily_file(input_dir, output_dir, year, month, day):
    ...
    goodkeys = ['corrected_differential_reflectivity',
                ...
                'NW', 'corrected_reflectivity', 'region_dealias_velocity',
    ...
    for cnt, input_file in enumerate(flist):
        ...
        for mykey in goodkeys:
            rslt = read_extract(radar, mykey, fillvalue=FILLVALUE)
            rslt = np.ma.masked_where(np.isnan(rslt) | (rslt == FILLVALUE), rslt)
            MOMENT[mykey][cnt, :, :] = rslt
